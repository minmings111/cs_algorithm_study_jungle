...
def merge(arr, left, mid, right):
    ...
    l_p = 0
    r_p = 0
    k = left # 원본 배열 arr에 값이 채워질 시작 위치
    ...
    while l_p < len(left_arr) and r_p < len(right_arr):
        ...
        elif left_arr[l_p] > right_arr[r_p]:
            arr[k] = right_arr[r_p]
            r_p += 1
            k += 1
    ...
    # 둘 중 한 쪽 배열이 먼저 다 사용되면 나머지 배열을 모두 넣어야 함
    if l_p == len(left_arr):
        for i in range(len(right_arr)-r_p):
            arr[k] = right_arr[r_p]
            r_p += 1
            k += 1

    elif r_p == len(right_arr):
        for i in range(len(left_arr)-l_p):
            arr[k] = left_arr[l_p]
            l_p += 1
            k += 1

    # # 더 보편적인 정석 표현 (while 사용)
